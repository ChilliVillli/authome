...
url = 'https://www.che168.com/china/hafu/a0_0msdgscncgpi1ltocsp1exx0/'
...
def array():
    ...
    for url_work in get_url():
        ...
        photo_first = card_soup.find('a', class_='jiaodianphotoclick').find('img').get('src')
        sleep(2)
        page.write(f'Q{row + 1}', f"https:{photo_first}")
        photo_car = card_soup.find('div', class_='car-pic-list js-box-text').find_all('a')
        sleep(1.5)

        for pc in photo_car:
            b += f"https:{pc.find('img').get('data-original')};"
        page.write(f'R{row + 1}', b)
        b = ''
        ...
        for j in auto_card:
            ...
            for q in range(len(j.find_all('li'))):
                ...
                if '标准容量' in a:
                    ...
                    continue
                if '燃油标号' in a:
                    page.write(f"O{row}", a.replace('燃油标号', ''))
                    sleep(1)
                    column += 1
                    continue
